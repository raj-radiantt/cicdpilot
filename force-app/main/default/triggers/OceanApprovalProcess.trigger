trigger OceanApprovalProcess on Ocean_Request__c (After Update) {
    ...
    if(Trigger.isAfter && Trigger.isUpdate){
            ...
            // if(Trigger.old.Review_Outcome__c != 'Approved' && (oc.CRMT_Request_Status__c == CRMT_INTAKE_REVIEW || oc.CRMT_Request_Status__c == ATTESTATION))  {
            ...
            for(Ocean_Request__c oceanReq : Trigger.new){
                // if( OceanUpdateResourceStatus.isRecursive == true && Trigger.oldMap.get(oceanReq.id).Review_Outcome__c != 'Approved' && (Trigger.newmap.get(oceanReq.id).CRMT_Request_Status__c == 'CRMT Intake Review' || Trigger.newmap.get(oceanReq.id).CRMT_Request_Status__c == 'Attestation Requested'))  {
                //     updateHandler.updateResourceStatus(Trigger.New,Trigger.old);
                // }
